...
function renderDragSuspectsPage() {
  ...
  function showContentForOrientation() {
    if (window.matchMedia("(orientation: landscape)").matches) {
      ...
      dragSuspectsBackgroundOverlay.setAttribute(
        "id",
        "dragSuspectsBackgroundOverlay"
      );
...
function explainDraggingSuspects(event) {
  if (!document.querySelector("div#screenOverlayInfoButtonDragSuspectsPage")) {
    ...
    document
      .querySelector("div#screenOverlayInfoButtonDragSuspectsPage")
      .remove();
...
function drop(event) {
  ...
  if (
    dropZone.classList.contains("dropArea") &&
    ...
    for (let i = 0; i < event.currentTarget.children.length; i++) {
      if (
        event.currentTarget.children[i].classList.contains(
          "culpritFrameInDropArea"
        )
      ) {
    ...
    for (let i = 0; i < event.currentTarget.children.length; i++) {
      if (
        event.currentTarget.children[i].classList.contains("draggableImage")
      ) {
    ...
    for (let i = 0; i < suspects.length; i++) {
      if (
        !suspects[i].children[0].classList.contains("suspectDraggedToFrame")
      ) {
    ...
    if (allSuspectsPlaced === true) {
      ...
      document
        ...
        .appendChild(checkSuspectsArrow);
    }

    if (data === "drag0") {
      document.querySelector("p#name1DragSuspectsPage").style.visibility =
        "hidden";
    } else if (data === "drag1") {
      document.querySelector("p#name2DragSuspectsPage").style.visibility =
        "hidden";
    } else if (data === "drag2") {
      document.querySelector("p#name3DragSuspectsPage").style.visibility =
        "hidden";
    } else if (data === "drag3") {
      document.querySelector("p#name4DragSuspectsPage").style.visibility =
        "hidden";
    } else if (data === "drag4") {
      document.querySelector("p#name5DragSuspectsPage").style.visibility =
        "hidden";
    } else if (data === "drag5") {
      document.querySelector("p#name6DragSuspectsPage").style.visibility =
        "hidden";
    } else if (data === "drag6") {
      document.querySelector("p#name7DragSuspectsPage").style.visibility =
        "hidden";
    }
